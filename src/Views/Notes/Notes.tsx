...
//     try {
//       await deleteNote(uid, label, note.id);
...
//           <div
//             key={note.id}
//             className="p-4 bg-white shadow-md rounded-lg relative cursor-pointer"
    ...
    <div className="p-4">
      ...
      <div className="grid lg:grid-cols-3 gap-4 mt-12">
        {notes.map((note) => (
          <div
            key={note.id}
            className="p-4 bg-white shadow-md rounded-lg relative cursor-pointer overflow-hidden"
            onClick={() => handleNote(note)}
          >
            <button
              className="absolute top-2 right-2 p-1 rounded-full hover:bg-gray-200"
              onClick={(e) => handleDelete(e, note)}
            >
              <img className="w-4 h-4" src={ICONS.Menu} alt="Delete" />
            </button>
